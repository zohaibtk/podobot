...
TRANSIENT_GROK_STATUSES = {429, 500, 502, 503, 504}
LEGACY_GROK_MODEL_ALIASES = {
...
class GrokLLMProvider:
    def __init__(
        ...
        *,
        ...
        self.model = self._normalized_model(model or settings.grok_model)
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        if not self.api_key:
            raise RuntimeError("Grok API key must be configured in Integrations.")
        ...
        async with httpx.AsyncClient(
            ...
            for attempt in range(self.max_retries + 1):
                try:
                    ...
                    raise RuntimeError(
                        f"Grok request failed with status {exc.response.status_code}: {detail}"
                    ) from exc
                except httpx.HTTPError as exc:
                    if self._should_retry(None, attempt):
                        await self._sleep_before_retry(attempt)
                        continue
                    raise RuntimeError("Grok request failed") from exc

        raise RuntimeError("Grok request failed")

    def _should_retry(self, status_code: int | None, attempt: int) -> bool:
        if attempt >= self.max_retries:
            return False
        return status_code is None or status_code in TRANSIENT_GROK_STATUSES
    ...
    def _chat_completions_url(self) -> str:
        return f"{settings.grok_api_base_url.rstrip('/')}/chat/completions"

    def _normalized_model(self, model: str) -> str:
        return LEGACY_GROK_MODEL_ALIASES.get(model, model)
    ...
    def _error_detail(self, response: httpx.Response) -> str:
        try:
            ...
            return text[:240] if text else "No error detail returned by xAI."
    ...
    def _agent_prompt(self, request: LLMRequest) -> str:
        return (
            "You are the fallback production LLM provider for PodoBot. "
    ...
    def _extract_json(self, raw: Mapping[str, object]) -> dict[str, object]:
        ...
        try:
            ...
            return {"summary": text or "Grok returned no structured content."}
    ...
    def _fallback_summary(self, request: LLMRequest) -> str:
        return f"Grok generated workflow support for {request.agent_key}."
