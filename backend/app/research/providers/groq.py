...
class GroqResearchProvider(HTTPResearchProvider):
    ...
    async def test_connection(self):
        ...
        return self._test_success(message="Groq API is reachable.", latency_ms=220)
    ...
    def normalize(self, raw_result: Mapping[str, Any]) -> dict[str, object]:
        text = str(raw_result.get("text") or "No Groq classification text returned.")
        document = normalized_document(
            provider_type=self.provider_type,
            provider_mode=self.provider_mode,
            source_key=self.source_key,
            title="Groq research classification",
            content=text,
            snippet=text[:280],
    ...
    def calculate_provider_scores(self, normalized_result: Mapping[str, Any]) -> dict[str, object]:
        return self._scores(normalized_result, engagement=0, authority=0.76)
